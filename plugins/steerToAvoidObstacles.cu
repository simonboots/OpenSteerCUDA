...
steerToAvoidObstacles(VehicleData* vehicleData, float3 *steeringVectors)
{
    ...
    for (; i < numOfObstacles; i++) {
        ...
        LC(threadIdx.x) = make_float3(float3Dot(LC(threadIdx.x), (*vehicleData).side[id]),
                                      float3Dot(LC(threadIdx.x), (*vehicleData).up[id]),
                                      float3Dot(LC(threadIdx.x), (*vehicleData).forward[id]));
        
        // compute line-sphere intersection parameters
        b = -2*LC(threadIdx.x).z;
        c = LC(threadIdx.x).x*LC(threadIdx.x).x + LC(threadIdx.x).y*LC(threadIdx.x).y + LC(threadIdx.x).z*LC(threadIdx.x).z - (obstacles[i].radius + (*vehicleData).radius[id])*(obstacles[i].radius + (*vehicleData).radius[id]);
        d = (b * b) - (4 * c);
